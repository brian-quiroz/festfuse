...
heuristic — is in **`docs/process/artist-editorial-process.md`**. Read it in full once
when you start a stretch of artist work, for the reasoning and the per-field rules. The
output gate at the bottom of this file is what you run before every review batch; it is
short by design, re-read it each time. Incident case studies are in
`docs/process/artist-editorial-incidents.md`; read those only when a check is getting
shaky.
