...
// the same gap exists anywhere decisionStore/scheduleStore-derived state renders (Sidebar
// counts, Explore's pick/schedule buttons, the Planner grid's own coloring). A single
// shared gate covers all of them at once rather than repeating this check in every
// consumer. The gap is only ever on a hard reload — client-side navigation never remounts
// these stores, so this never affects normal in-app browsing.
export default function HydrationGate({ children }: { children: React.ReactNode }) {
  ...
  if (!decisionHydrated || !scheduleHydrated || !plannerViewHydrated) {
    return null;
  }
