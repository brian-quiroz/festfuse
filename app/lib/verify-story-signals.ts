...

{
  ...
  check(
    "all days selected: eligible pool equals full lineup",
    eligible.length === allArtists.length,
    `${eligible.length}/${allArtists.length}`
  );
}
{
  const eligible = getEligibleArtists(ACTIVE_FESTIVAL_ID, ["Friday"], allArtists);
  const expected = allArtists.filter(
    (a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Friday"]) !== undefined
  );
  check(
    "one selected day: eligible pool matches direct resolution",
...
}
{
  ...
  check(
    "nonconsecutive days (Thu+Sun): excludes artists only on Fri/Sat",
    onlyFriSat.every((a) => !eligible.includes(a))
  );
}
{
  const eligible = getEligibleArtists(ACTIVE_FESTIVAL_ID, [], allArtists);
  check(
    "explicit zero attendance days: eligible pool is empty, not the full lineup",
    eligible.length === 0,
    `${eligible.length}`
  );
  const signals = run(
    [],
    decisionsFor(
      allArtists.slice(0, 10).map((a) => a.slug),
      "mustSee"
    )
  );
  check(
    "zero attendance days: computeStorySignals returns no cards (nothing is eligible)",
    signals.length === 0
  );
}
{
  const decisions = decisionsFor(
    allArtists
      .filter((a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Friday"]))
      .slice(0, 6)
      .map((a) => a.slug),
    "mustSee"
  );
...
{
  const fridayEligible = allArtists.filter(
    (a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Friday"]) !== undefined
  );
  const decisions = {
    ...decisionsFor(
      fridayEligible.slice(0, 3).map((a) => a.slug),
      "mustSee",
      "explore"
    ),
    ...decisionsFor(
      fridayEligible.slice(3, 6).map((a) => a.slug),
      "interested",
      "quickPicks"
    ),
  };
  ...
  check(
    "Explore-sourced decision counts as a valid positive pick",
    picks.length === 6,
    `${picks.length}`
  );
}
{
  const fridayOnly = allArtists.filter(
    (a) =>
      getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Friday"]) !== undefined &&
      getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Thursday"]) === undefined
  );
  const decisions = decisionsFor(
    fridayOnly.slice(0, 5).map((a) => a.slug),
    "mustSee"
  );
  const picksOnThursday = getValidPositivePicks(
    ACTIVE_FESTIVAL_ID,
    ["Thursday"],
    allArtists,
    decisions
  ...
  check(
    "decisions for artists outside the selected days do not count",
    picksOnThursday.length === 0,
    `${picksOnThursday.length}`
  );
}
{
...
}
{
  ...
  check(
    "multi-appearance artist (devault) counted exactly once",
    devaultCount === 1,
    `count=${devaultCount}`
  );
}
{
  const primaryExcluded = allArtists.find(
    (a) =>
      getSelectedDayAppearance(
        a,
        ACTIVE_FESTIVAL_ID,
        ALL_DAYS.filter((d) => d !== "Friday")
      ) !== undefined
  )!;
  check(
    "fixture: found an artist eligible when Friday (an arbitrary day) is excluded",
    !!primaryExcluded
  );
}
...
for (let n = 1; n <= 4; n++) {
  ...
  check(
    `${n} valid positive pick(s) -> []`,
    run(ALL_DAYS, decisionsFor(picks, "mustSee")).length === 0
  );
}
{
  ...
  check(
    "exactly 5 valid positive picks -> exactly 4 cards",
    signals.length === 4,
    `${signals.length}`
  );
}
{
  ...
  const decisions = {
    ...
    ...decisionsFor(
      fridayOnly.slice(0, 2).map((a) => a.slug),
      "interested"
    ),
  };
  const signals = run(["Thursday"], decisions);
  check(
    "5 positive decisions, 2 outside attendance scope -> locked (< 5 valid)",
    signals.length === 0
  );
}
{
  ...
  check(
    "nearly the entire eligible lineup selected -> still exactly 4 cards, no NaN",
    signals.length === 4 && signals.every((s) => Number.isFinite(s.userValue ?? 0))
  );
}
...
{
  const satEligible = allArtists.filter(
    (a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Saturday"]) !== undefined
  );
  const satNonChicago = satEligible.filter((a) => a.location.city !== "Chicago");
  const chicagoRate =
    (satEligible.filter((a) => a.location.city === "Chicago").length / satEligible.length) * 100;
  check(
    "fixture: Saturday eligible Chicago baseline clears the old 12pp threshold territory",
    chicagoRate > 12,
    `${chicagoRate.toFixed(1)}%`
  );
  const signals = run(
    ["Saturday"],
    decisionsFor(
      satNonChicago.slice(0, 8).map((a) => a.slug),
      "mustSee"
    )
  );
  check(
    "zero Chicago picks against a baseline above the old 12pp threshold -> no hometown card",
    !dims(signals).includes("hometown"),
    dims(signals).join(",")
  );
}
{
  ...
  const signals = run(
    ["Saturday"],
    decisionsFor(
      [...oneChicago, ...rest].map((a) => a.slug),
      "mustSee"
    )
  );
  check(
    "exactly one Chicago pick -> no hometown card (below the 2-pick floor)",
    !dims(signals).includes("hometown"),
    dims(signals).join(",")
  );
}
{
  ...
  const signals = run(
    ALL_DAYS,
    decisionsFor(
      [...chicagoArtists.slice(0, 5), ...others.slice(0, 3)].map((a) => a.slug),
      "mustSee"
    )
  );
  const hometown = findType(signals, "hometown");
  check(
    "2+ Chicago picks with meaningful over-index -> hometown card present",
    !!hometown,
    dims(signals).join(",")
  );
  check(
    ...
    !hometown ||
      (hometown.userValue !== undefined &&
        hometown.lineupValue !== undefined &&
        hometown.userValue > hometown.lineupValue)
  );
}
{
  ...
  check(
    "Chicago under-indexing (0 of 20 picks) -> no hometown card of any kind",
    !dims(signals).includes("hometown")
  );
}
{
  ...
  check(
    "international over-indexing -> international card present, positive direction",
    !!intl && (intl.userValue ?? 0) > (intl.lineupValue ?? 0),
    dims(signals).join(",")
  );
}
{
  ...
  check(
    "zero international picks -> no international card",
    !dims(signals).includes("international")
  );
}
{
  const usArtists = allArtists.filter((a) => a.location.country === "United States");
  ...
  const signals = run(
    ALL_DAYS,
    decisionsFor(
      [...shuffled(usArtists, 11).slice(0, 18), ...intlArtists.slice(0, 2)].map((a) => a.slug),
      "mustSee"
    )
  );
  check(
    "international under-indexing -> no international card (never an inverse 'American-heavy' card)",
    !dims(signals).includes("international")
  );
}
{
  ...
  const signals = run(
    ALL_DAYS,
    decisionsFor(
      [...headliners.slice(0, 8), ...undercard.slice(0, 2)].map((a) => a.slug),
      "mustSee"
    )
  );
  const billing = findType(signals, "billing");
  check(
    "headliner-heavy picks -> billing card, headliner-heavy copy",
    billing?.headlineTemplate === "Big names lead the way",
    billing?.headlineTemplate
  );
  ...
  const signals2 = run(
    ALL_DAYS,
    decisionsFor(
      allUndercardPicks.map((a) => a.slug),
      "mustSee"
    )
  );
  const billing2 = findType(signals2, "billing");
  check(
    "undercard-heavy picks -> billing card, undercard-heavy copy",
    billing2?.headlineTemplate === "Heavy on the undercard",
    billing2?.headlineTemplate ?? dims(signals2).join(",")
  );
}
{
  ...
  const focusedSignals = run(
    ALL_DAYS,
    decisionsFor(
      focusedPicks.map((a) => a.slug),
      "mustSee"
    )
  );
  ...
  check(
    ...
    !focusedStage ||
      focusedStage.headlineTemplate === "Anchored to a few stages" ||
      focusedStage.headlineTemplate === "Your festival footprint",
  ...
  const broadSignals = run(
    ALL_DAYS,
    decisionsFor(
      broadPicks.map((a) => a.slug),
      "mustSee"
    )
  );
  ...
  check(
    ...
    !broadStage ||
      broadStage.headlineTemplate === "Stages all across the park" ||
      broadStage.headlineTemplate === "Your festival footprint",
    broadStage?.headlineTemplate ?? "(none)"
  );
}
{
  const focusedGenrePicks = allArtists
    .filter((a) => a.genres.every((g) => a.genres[0] === g))
    .slice(0, 15);
  if (focusedGenrePicks.length >= 5) {
    const signals = run(
      ALL_DAYS,
      decisionsFor(
        focusedGenrePicks.map((a) => a.slug),
        "mustSee"
      )
    );
...
}
{
  ...
  const edmOnly = allArtists.filter(
    (a) => a.genres.length > 0 && a.genres.every((g) => GENRE_TO_FAMILY[g] === "Dance/Electronic")
  );
  const signals = run(
    ALL_DAYS,
    decisionsFor(
      edmOnly.slice(0, 20).map((a) => a.slug),
      "mustSee"
    )
  );
  const taste = findType(signals, "genreAffinity");
  check(
    "genuinely concentrated genre family qualifies for strong interpretive copy",
    isStrongTasteCopy(taste?.headlineTemplate),
    taste?.headlineTemplate
  );
}
{
...
}
{
  ...
  const fixedFamilyRate = (artists: Artist[]) =>
    artists.length === 0
      ? 0
      : (artists.filter((a) => a.genres.some((g) => GENRE_TO_FAMILY[g] === "Pop")).length /
          artists.length) *
        100;
  const trueMaxFamilyRate = (artists: Artist[]) => {
    const presence: Record<string, number> = {};
    artists.forEach((a) =>
      new Set(a.genres.map((g) => GENRE_TO_FAMILY[g])).forEach(
        (f) => (presence[f] = (presence[f] ?? 0) + 1)
      )
    );
...
}
{
  ...
  const taste = findType(signals, "genreAffinity");
  check(
    "two-way leading-family tie -> tied Taste Profile copy, names both families",
    taste?.headlineTemplate === "Your leading sounds" &&
      (taste?.supportingText.includes("Americana and Pop") ?? false),
...
}
{
  ...
  check(
    ...
    taste?.headlineTemplate === "Your leading sounds" &&
      taste?.supportingText ===
        "Americana, Dance/Electronic, Hip-Hop/Rap, and 1 more are running neck and neck across your festival picks.",
    taste?.supportingText
  );
}
{
  ...
  check(
    ...
    taste?.headlineTemplate === "Your leading sounds" &&
      taste?.supportingText ===
        "Americana, Dance/Electronic, Hip-Hop/Rap, and 2 more are running neck and neck across your festival picks.",
    taste?.supportingText
  );
}
{
...
{
  const picks = allArtists
    .filter((a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Friday"]) !== undefined)
    .slice(0, 8);
  const signals = run(
    ["Friday"],
    decisionsFor(
  ...
  check(
    "one selected attendance day -> no day interpretive signal",
    !dims(signals).includes("day"),
    dims(signals).join(",")
  );
}
{
  const thuArtists = allArtists.filter(
    (a) =>
      getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Thursday", "Sunday"])?.day === "Thursday"
  );
  const sunArtists = allArtists.filter(
    (a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ["Thursday", "Sunday"])?.day === "Sunday"
  );
  const signals = run(
    ["Thursday", "Sunday"],
    decisionsFor(
      [...thuArtists.slice(0, 8), ...sunArtists.slice(0, 1)].map((a) => a.slug),
      "mustSee"
    )
  );
...
}
{
  ...
  const signals = run(
    ["Thursday", "Friday"],
    decisionsFor(
      proportionalPicks.map((a) => a.slug),
      "mustSee"
    ),
    dayPool
  );
...
}
{
  ...
  const devaultAppearance = getSelectedDayAppearance(
    eligible.find((a) => a.slug === "devault")!,
    ACTIVE_FESTIVAL_ID,
    ["Thursday", "Friday"]
  );
  check(
    "devault (multi-appearance) resolves to exactly one day for the day signal",
    devaultAppearance?.day === "Thursday",
    devaultAppearance?.day
  );
}
...

{
  ...
  check(
    "identical scope/sample-size/eligible-lineup -> identical seed (deterministic)",
    seed1 === seed2
  );
}
{
  ...
  const eligible = getEligibleArtists(ACTIVE_FESTIVAL_ID, ALL_DAYS, allArtists);
  const picksA = shuffled(eligible, 41)
    .slice(0, 15)
    .map((a) => a.slug);
  const picksB = shuffled(eligible, 42)
    .slice(0, 15)
    .map((a) => a.slug);
  check(
    "fixture: two independently-shuffled 15-artist pick sets are actually different",
    JSON.stringify([...picksA].sort()) !== JSON.stringify([...picksB].sort())
  );
  ...
  check(
    "same scope/sample-size/eligible-lineup produces the same seed regardless of which artists were picked",
    seedA === seedB
  );
}
{
...
{
  const seedSmall = buildStorySeed(
    ACTIVE_FESTIVAL_ID,
    ["Friday"],
    6,
    allArtists.slice(0, 30).map((a) => a.slug)
  );
  const seedLarge = buildStorySeed(
    ACTIVE_FESTIVAL_ID,
    ["Friday"],
    6,
    allArtists.slice(0, 31).map((a) => a.slug)
  );
  check("different eligible-lineup fingerprint changes the seed", seedSmall !== seedLarge);
}
{
  const decisions = decisionsFor(
    shuffled(allArtists, 55)
      .slice(0, 30)
      .map((a) => a.slug),
    "mustSee"
  );
  ...
  check(
    "identical inputs produce byte-identical cards, copy, and order across repeated calls",
...
}
{
  ...
  check(
    "changing attendance scope changes the computed Story",
...

{
  const scenarios: Record<string, ArtistDecision>[] = [
    decisionsFor(
      allArtists.slice(0, 5).map((a) => a.slug),
      "mustSee"
    ),
    decisionsFor(
      shuffled(allArtists, 1)
        .slice(0, 15)
        .map((a) => a.slug),
      "mustSee"
    ),
    decisionsFor(
      shuffled(allArtists, 2)
        .slice(0, 40)
        .map((a) => a.slug),
      "interested"
    ),
    decisionsFor(
      shuffled(allArtists, 3)
        .slice(0, 80)
        .map((a) => a.slug),
      "mustSee"
    ),
  ...
  check(
    "Decision Profile is always included and always the final insight",
    allHaveDecisionProfileLast
  );
  ...
  check(
    "no journeySummary dimension anywhere",
    scenarios.every((d) => !dims(run(ALL_DAYS, d)).includes("journeySummary"))
  );
}
{
  const intlArtists = allArtists.filter((a) => a.location.country !== "United States");
  ...
  check(
    "international and countryDiversity are never both selected",
    !(hasIntl && hasCountry),
    dims(signals).join(",")
  );
}
{
  ...
  check(
    "genreAffinity(strong) and genreBreadth can coexist in the same Story",
    taste?.lineupValue !== undefined && !!breadth,
    dims(signals).join(",")
  );
}
{
...

{
  ...
  function decisionProfileFor(mustSeeCount: number, interestedCount: number) {
    const decisions = {
      ...decisionsFor(
        pool.slice(0, mustSeeCount).map((a) => a.slug),
        "mustSee"
      ),
      ...decisionsFor(
        pool.slice(mustSeeCount, mustSeeCount + interestedCount).map((a) => a.slug),
        "interested"
      ),
    };
  ...
  check(
    "5 picks (5/0) -> restrained copy",
    decisionProfileFor(5, 0)?.headlineTemplate === "Your picks are taking shape",
    decisionProfileFor(5, 0)?.headlineTemplate
  );
  check(
    "7 picks (mixed) -> restrained copy",
    decisionProfileFor(4, 3)?.headlineTemplate === "Your picks are taking shape",
    decisionProfileFor(4, 3)?.headlineTemplate
  );

  // 8 picks at 25% (2/8), 40% (not achievable at exactly 8 with integers close: use 3/8=37.5%), 60% (5/8=62.5%... need exact boundary values)
  check(
    "8 picks at 25% Must See (2/8) -> Interested-heavy copy",
    decisionProfileFor(2, 6)?.headlineTemplate === "Scouting mode active",
    decisionProfileFor(2, 6)?.headlineTemplate
  );
  check(
    "8 picks at 75% Must See (6/8) -> Must-See-heavy copy",
    decisionProfileFor(6, 2)?.headlineTemplate === "Heavy on the non-negotiables",
    decisionProfileFor(6, 2)?.headlineTemplate
  );

  // Use a 10-pick pool for clean 40%/60% boundary values.
  check(
    "10 picks at 40% Must See (4/10, near-even boundary) -> near-even copy",
    decisionProfileFor(4, 6)?.headlineTemplate === "Balanced between priority and curiosity",
    decisionProfileFor(4, 6)?.headlineTemplate
  );
  check(
    "10 picks at 60% Must See (6/10, near-even boundary) -> near-even copy",
    decisionProfileFor(6, 4)?.headlineTemplate === "Balanced between priority and curiosity",
    decisionProfileFor(6, 4)?.headlineTemplate
  );
  check(
    "10 picks at 50% Must See -> near-even copy",
    decisionProfileFor(5, 5)?.headlineTemplate === "Balanced between priority and curiosity",
    decisionProfileFor(5, 5)?.headlineTemplate
  );
  ...
  check(
    "11 picks at ~64% Must See (7/11, Must-See-leaning middle) -> shortlist copy",
    decisionProfileFor(7, 4)?.headlineTemplate === "Priorities set, curiosity intact",
    decisionProfileFor(7, 4)?.headlineTemplate
  );
  check(
    "11 picks at ~36% Must See (4/11, Interested-leaning middle) -> discovery copy",
    decisionProfileFor(4, 7)?.headlineTemplate === "Room for discovery",
    decisionProfileFor(4, 7)?.headlineTemplate
  );

  const extreme9 = decisionProfileFor(8, 1); // 9 total, 88.9% -> heavy but not extreme (count<10)
  check(
    "9 picks at ~89% Must See -> heavy but NOT extreme (below the 10-pick floor)",
    extreme9?.headlineTemplate === "Heavy on the non-negotiables",
    extreme9?.headlineTemplate
  );

  const extreme10 = decisionProfileFor(9, 1); // 10 total, 90% -> extreme
  check(
    "10 picks at 90% Must See -> extreme copy",
    extreme10?.headlineTemplate === "Zero hesitation",
    extreme10?.headlineTemplate
  );

  const extremeInterested10 = decisionProfileFor(1, 9); // 10 total, 10% Must See -> extreme interested
  check(
    "10 picks at 10% Must See -> extreme Interested copy",
    extremeInterested10?.headlineTemplate === "Keeping every door wide open",
    extremeInterested10?.headlineTemplate
  );
}
...

{
  ...
  const headliners = allArtists.filter(
    (a) => getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ALL_DAYS)?.billingTier === "Headliner"
  );
  const subHeadliners = allArtists.filter(
    (a) =>
      getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ALL_DAYS)?.billingTier === "Sub-headliner"
  );
  const undercard = allArtists.filter((a) => {
    const t = getSelectedDayAppearance(a, ACTIVE_FESTIVAL_ID, ALL_DAYS)?.billingTier;
    return t !== "Headliner" && t !== "Sub-headliner";
  ...
  const picks = [
    ...headliners.slice(0, 1),
    ...subHeadliners.slice(0, 1),
    ...shuffled(undercard, 61).slice(0, 3),
  ];
  const signals = run(
    ALL_DAYS,
    decisionsFor(
  ...
  check(
    ...
    billing?.supportingText ===
      "1 headliner, 1 sub-headliner, and 3 undercard acts make up your current list.",
    billing?.supportingText
  );
}
{
  ...
  const scenarios: Record<string, ArtistDecision>[] = [
    ...
    decisionsFor(
      shuffled(allArtists, 4)
        .slice(0, 8)
        .map((a) => a.slug),
      "mustSee"
    ),
    decisionsFor(
      shuffled(allArtists, 6)
        .slice(0, 60)
        .map((a) => a.slug),
      "interested"
    ),
