...
Nothing in this roadmap is started. The backend hierarchy already supports it: the
run-scoped read API (`/festivals/{edition}/runs/{run}/…`) resolves a run by
...
videos are global and reusable across editions and runs. The gaps are: seeding is
hard-coded to Lollapalooza and never seeds stages; no authoring path adds an existing
artist to a new run; the bulk appearances feed cannot surface an announced entry with
no scheduled appearance; and the entire frontend is wired to one hard-coded festival
and run with no festival/run in any URL and picks keyed by artist slug alone.
...
- `backend/scripts/seed_festival.py` becomes config-driven: a per-edition config
  (small data module, or one file per edition) rather than inline Lollapalooza
  literals. It seeds series → edition → every run → days **and stages**. Stages are
  currently seeded by nothing — only `pg_restore` of an existing dump — so a new
  edition's stage set has no seed path today. Idempotency becomes per-entity (per run,
  day, stage), not whole-edition.
- Add the ACL 2026 config: series `austin-city-limits`, edition `acl-2026` (Austin,
  `America/Chicago`), runs `weekend-1` / `weekend-2` with their day dates, and the ACL
  stage set.
- Any genre the ACL lineup needs that the `genres` table lacks is a two-file change
  (`genres` row via an autogenerated migration, plus `app/data/categories.ts`) — the
  existing "Genre Vocabulary Lives in Two Places" future consideration.
- Update `backend/tests/integration/test_clean_bootstrap.py` (it asserts one edition,
  one run, four days) and add a `seed_festival` unit test.

**Checkpoint:** ACL 2026's hierarchy (two runs, days, stages) seeds idempotently on
the local and hosted database.
...
- Festival, run, day, and stage creation stay `seed_festival.py`'s responsibility.
