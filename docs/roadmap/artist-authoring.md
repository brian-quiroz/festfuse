...
- The genre CLIs (`add-genre` / `delete-genre`) are section 7; `add-artist` errors
  clearly if a genre is missing.
...
- Introducing a genre is still flagged with sources in the research report and stays a
  two-place change: the `genres` table (via `add-genre`, section 7) and the
  `app/data/categories.ts` filter vocabulary (a hand edit).
...
- `app/services/genre_authoring.py` (`create_genre` / `delete_genre` / the pure
  ...
  as `add_artist`. `create_genre` is idempotent — re-adding an identical row is a
  no-op, so the same invocation replays against the hosted database.
- `add_genre --name "New Wave" --family "Rock"` resolves the family by name, derives
  the slug the way the seeded rows are slugged (overridable with `--slug`), rejects a
  name or slug already taken by a different row, and prints the matching
  `app/data/categories.ts` lines to add by hand.
- `delete_genre --slug <slug>` refuses a genre any artist is assigned; it exists to
  keep `add_genre` testable without accumulating rows, mirroring `delete_artist`.
- The frontend filter vocabulary in `app/data/categories.ts` stays a separate hand
  edit (ADR-0011: the `genres` table and `categories.ts` "are not required to converge
  until the filter list itself is served from the API"). The `genres`-table row and
  the `categories.ts` entry are still applied in the same change.
- Coverage: `test_genre_authoring_schema.py` (fast) for the slug derivation and the
  pre-query guards; `integration/test_genre_authoring.py` (PostgreSQL) for family
  resolution, name/slug clash detection, the idempotent re-add, and the
  assigned-genre delete guard.
